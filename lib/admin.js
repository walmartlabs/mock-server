...
module.exports = function(app) {
  app.get('/ms_admin', function(req, res, next) {
    ...
    async.parallel([
      function(callback) {
        childProcess.exec('git for-each-ref --format="%(refname)" refs/heads/', {cwd: config.appDir}, function(err, stdout, stderr) {
          branches = stdout.split('\n')
              .filter(function(branch) { return branch; })
              .map(function(branch) { return branch.replace(/^refs\/heads\//, ''); })
              .sort();
          callback(err);
        });
      },
