...
module.exports = function(app) {
    ...
    async.series([
      ...
      function(callback) {
        repo.pull(function(err) {
          // Do not ignore up to date errors that are on the current branch
          if (!sameBranch && err && err.upToDate) {
            err = undefined;
          }
          callback(err);
        });
  ...
  app.all('/ms_admin/pull', express.bodyParser(), function(req, res, next) {
    ...
    function(err) {
      ...
      if (campfireRoom && !err) {
        campfireRoom.speak('mock-server ' + req.headers.host + ' pulled from ' + currentBranch);
      }
      res.redirect('/ms_admin');
    });
  });
