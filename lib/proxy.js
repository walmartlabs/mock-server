...
exports.proxy = function(secure) {
  ...
  return function(req, res) {
    ...
    var originalHost = options.headers.host;
    options.headers.host = options.host;

    if (config.logTrace && options.headers['accept-encoding']) delete options.headers['accept-encoding'];
    ...
    var sourceReq = (secure || forwarded ? https : http).request(options, function(sourceRes) {
        ...
        cors.applyCORS(originalHost, req, sourceRes);
        ...
        if (sourceRes.statusCode === 302) {
          ...
          if (redirect.hostname === host || redirect.hostname === config.proxyServer) {
            ...
            redirect.host = redirect.hostname + ':' + redirect.forwardPort;
